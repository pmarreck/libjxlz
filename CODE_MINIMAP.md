...
- `enc_encoding.zig` — first modular encoder tokenization slice: converts a single grayscale channel plus single-node non-weighted predictor into packed residual `Token`s, with exact-value tests for `zero` and `gradient` predictor behavior
