...
- `enc_frame.zig` — first encoder-side frame wrapper, currently built around borrowing a known-good grayscale frame-header bit prefix and appending freshly generated TOC + section bytes; includes the first frame-level grayscale image roundtrip through `FrameDecoder.decodeFrame`
- `enc_codestream.zig` — first encoder-side full codestream wrapper, currently built around a real raw `SizeHeader` writer plus borrowed grayscale metadata bits and the borrowed-header frame wrapper; includes the first full grayscale codestream roundtrip through header parsing and `FrameDecoder.decodeFrame`
...
- `enc_encoding.zig` — first modular encoder tokenization/writing slice: converts a single grayscale channel plus single-node non-weighted predictor into packed residual `Token`s, feeds them into the encoder-side ANS writer, can emit the smallest real global-tree group and a widened fully local-tree modular group using flat one-context channel histograms, and includes exact token assertions plus `modularDecode`-level roundtrip tests for `.zero` and `.gradient` local-tree paths
