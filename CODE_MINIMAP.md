...
- `tests/cli/encode_codestream_bench_smoke.sh` — checksum smoke test proving the real narrow encode harness compiles in ReleaseFast, cross-compiles cleanly for `x86_64-linux`, and emits the expected deterministic codestream checksum
...
- `enc_ans.zig` — first encoder-side entropy-writing foundations: generic `encodeUintConfig(s)` and `storeVarLenUint8/16`, `SizeWriter`, `Token`, `ANSEncSymbolInfo`, an alias-table-backed ANS info-table builder, `ANSCoder.putSymbol`, minimal one-context degenerate/two-symbol/flat histogram metadata writers plus multi-context flat-histogram bundles for both the all-zero and simple-direct context-map cases, a general contextual token writer keyed by `context_map[token.context]`, and a minimal single-histogram token-stream writer that now packs reversed ANS/extra-bit chunks through a single fixed-capacity chunk slice and pre-reserves the full outgoing bit burst before flushing chunks after real encode profiling identified `writeSingleHistogramTokens` / `BitWriter.write` growth churn as the current hotspot; tests prove metadata roundtrips, exact bit-count agreement, ANS state-update correctness, recovered histogram frequencies via alias tables, and decoder roundtrip of one- and multi-histogram token streams
- `enc_context_map.zig` — encoder-side multi-context metadata helpers: the original all-zero simple map writer plus direct-entry simple context maps for small histogram counts (`bits_per_entry` 1..3), both proven through `decodeContextMap` roundtrips; this is the current write-side foundation for multi-histogram MA-tree streams
...
- `enc_codestream.zig` — first encoder-side full codestream wrapper with native `SizeHeader`, `ImageMetadata`, `CustomTransformData`, and `enc_frame.writeFrame` serializers for the current simple grayscale/RGB modular path; now includes full codestream roundtrips through header parsing and `FrameDecoder.decodeFrame` for grayscale, tiny RGB, a real `600x300` multi-group RGB image using local trees, a matching `600x300` roundtrip that carries an encoded global `gradient_fixed_dc` MA tree plus shared histograms through the DC-global section, a second `600x300` roundtrip with a channel-split global tree that exercises two different histogram contexts during decode, and a third `600x300` roundtrip whose group payloads choose histogram contexts per pixel from a real MA-tree gradient-property split
...
- `enc_ma.zig` — encoder-side MA-tree writer now widened from the old single-leaf special case to a generic tree writer: it transliterates upstream `TokenizeTree` breadth-first token order into Zig, rebuilds the decoder-visible node layout while tokenizing, writes the current shared-histogram/context-map tree bitstream shape, and keeps `writeSingleLeafTree` as a thin wrapper; tests now cover both single-leaf and split-tree roundtrips through `dec_ma.decodeTree`
