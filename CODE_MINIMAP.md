...
- `enc_ans.zig` — first encoder-side entropy-writing foundations: generic `encodeUintConfig(s)` and `storeVarLenUint8/16`, `SizeWriter`, `Token`, `ANSEncSymbolInfo`, an alias-table-backed ANS info-table builder, `ANSCoder.putSymbol`, a minimal one-context degenerate-histogram metadata writer for `decodeHistograms`, and a minimal single-histogram token-stream writer; tests prove metadata roundtrips, exact bit-count agreement, ANS state-update correctness, and decoder roundtrip of both histogram metadata and direct/extra-bit token streams
