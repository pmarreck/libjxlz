...
- `enc_encoding.zig` — first modular encoder tokenization/writing slice: converts single-node non-weighted predictor channels into packed residual `Token`s, feeds them into the encoder-side ANS writer, can emit the smallest real global-tree group plus fully local-tree groups for grayscale, small RGB, and cropped/group-tile RGB images using flat one-context channel histograms plus bounded `HybridUintConfig` fallback when raw residual tokens exceed 8-bit direct alphabets; now includes a small reusable `FlatHistogramInfoCache` so repeated cropped/group tiles can reuse ANS info tables instead of rebuilding alias/reverse maps every time, and the hot rect path now tokenizes directly from source-channel windows instead of copying each tile into a temporary image first, with `modularDecode`-level roundtrip tests for `.zero`, `.gradient`, cropped RGB tiles, and the cache-backed rect path
