...
- `bench_modular_encode_codestream.zig` — Real narrow encoder benchmark harness for the working `600x300` multi-group RGB lossless modular path: assembles DC-global and local group sections, writes a full codestream, exposes deterministic `--print-checksum` / `--expect-checksum`, and serves as the new primary encode benchmark input for `./bm`
