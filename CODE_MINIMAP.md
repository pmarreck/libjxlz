...
- `enc_frame.zig` — first encoder-side frame wrapper, currently built around borrowing a known-good frame-header bit prefix and appending freshly generated TOC + section bytes; includes frame-level borrowed-shell roundtrips through `FrameDecoder.decodeFrame`
- `enc_codestream.zig` — first encoder-side full codestream wrapper, currently built around a real raw `SizeHeader` writer plus borrowed metadata bits and the borrowed-header frame wrapper; includes full borrowed-shell codestream roundtrips through header parsing and `FrameDecoder.decodeFrame` for both grayscale and RGB
