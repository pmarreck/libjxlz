...
| MA-SPLIT-RANGE | Each split leaves possible values on both branches under ancestor constraints | Upstream range propagation; independently checked encoded controls, siblings, property independence, signed limits and allocator sweeps | Native reversible bounds traversal and typed finding pass full tests/build/conformance September 12; exact normative clause mapping pending |
...
| CONTEXT-MAP-LZ77 | A nested map with at most two contexts cannot itself enable LZ77 | Upstream guard and Sneyers et al. section 8.2.1; depth-zero/one valid controls and depth-two invalid control | Native guard and typed finding pass full tests/build/conformance September 12; exact ISO clause pending |
| CONTEXT-MAP-CLUSTERS | Histogram IDs must cover the declared cluster range | Existing verification, valid/gapped maps and public corpus mutation | Specific invalid-context-map finding passes full tests/build/conformance September 12 |
| CONTEXT-MAP-BOUNDS | Simple-map entry reads must fit in the available bits | One-byte input requiring nine bits previously returned success | Explicit truncation check passes full tests/build/conformance September 12 |
| CONTAINER-VERSION | Unknown file-format versions cannot receive a complete validation claim | Assorted-corpus acceptance disagreement changes `ftyp` version to 16,711,680; versions 0/1 whole-codestream controls and unknown-version/public reset controls | Versions above 1 now report UNSUPPORTED/container_box; full tests/build/conformance pass September 12. Version 1 out-of-order partial codestream support remains open |
