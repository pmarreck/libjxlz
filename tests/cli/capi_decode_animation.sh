...
BUILD_LOG="${TMPDIR}/capi_decode_animation_build.log"
RUN_STDOUT="${TMPDIR}/capi_decode_animation_stdout.txt"
RUN_STDERR="${TMPDIR}/capi_decode_animation_stderr.txt"
...
if ! grep -Eq '^100 1 0 0 4 4 300 100 300 100$' "${RUN_STDOUT}"; then
	echo "unexpected decoder animation metadata"
	cat "${RUN_STDOUT}"
	exit 1
fi
