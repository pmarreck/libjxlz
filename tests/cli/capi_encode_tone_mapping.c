...
int main(void) {
	...
	info.alpha_bits = 0;
	info.intensity_target = 203.0f;
	info.min_nits = 1.5f;
	info.relative_to_max_display = 1;
	info.linear_below = 0.25f;
	...
	if (
		decoded_info.intensity_target != 203.0f ||
		decoded_info.min_nits != 1.5f ||
		decoded_info.relative_to_max_display != 1 ||
		decoded_info.linear_below != 0.25f
	) {
		fprintf(stderr, "tone mapping metadata mismatch\n");
		free(decoded_pixels);
		JxlDecoderDestroy(dec);
		free(encoded);
		return 1;
