...
int main(void) {
	...
	const uint8_t alpha_pixels[4] = { 255, 128, 64, 0 };
	const uint8_t expected_rgba[16] = {
	...
	for (;;) {
		...
		if (status == JXL_DEC_NEED_IMAGE_OUT_BUFFER) {
			if (JxlDecoderImageOutBufferSize(dec, &rgba_format, &decoded_size) != JXL_DEC_SUCCESS) {
			...
			if (JxlDecoderSetImageOutBuffer(dec, &rgba_format, decoded_pixels, decoded_size) != JXL_DEC_SUCCESS) {
	...
	if (decoded_info.xsize != 2 || decoded_info.ysize != 2 || decoded_info.alpha_bits != 8) {
	...
	if (decoded_size != sizeof(expected_rgba) || memcmp(decoded_pixels, expected_rgba, sizeof(expected_rgba)) != 0) {
