...
TASK_TMP="$(mktemp -d "${TMPDIR}/capi_container.XXXXXXXX")" || exit 1
...
if ! clang -std=c11 -Wall -Wextra -Werror \
	...
	tests/cli/capi_container.c "${PACKAGE_OUT}/lib/libjxlz_capi.a" \
	$(pkg-config --libs libbrotlienc libbrotlidec libbrotlicommon) \
	-o "${CHECK_BIN}" >"${BUILD_LOG}" 2>&1; then
	cat "${BUILD_LOG}" >&2
	exit 1
fi
if ! "${CHECK_BIN}" >"${TASK_TMP}/stdout" 2>"${TASK_TMP}/stderr"; then
