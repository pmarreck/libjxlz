...
int main(void) {
	...
	const uint8_t depth_pixels[2] = { 100, 25 };
	JxlPixelFormat rgb_format = {3, JXL_TYPE_UINT8, JXL_NATIVE_ENDIAN, 0};
	JxlPixelFormat depth_format = {1, JXL_TYPE_UINT8, JXL_NATIVE_ENDIAN, 0};
	...
	if (JxlEncoderSetExtraChannelInfo(enc, 0, &depth) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set extra channel info failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	if (JxlEncoderSetExtraChannelName(enc, 0, "depth-half", 10) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set extra channel name failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	...
	if (JxlEncoderSetExtraChannelBuffer(settings, &depth_format, depth_pixels, sizeof(depth_pixels), 0) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set extra channel buffer failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	...
	JxlExtraChannelInfo decoded_extra;
	memset(&decoded_extra, 0, sizeof(decoded_extra));
	if (JxlDecoderGetExtraChannelInfo(dec, 0, &decoded_extra) != JXL_DEC_SUCCESS) {
	...
	if (
		...
		decoded_info.alpha_bits != 0 ||
		decoded_extra.type != JXL_CHANNEL_DEPTH ||
		decoded_extra.dim_shift != 1
	) {
		fprintf(stderr, "unexpected decoded extra-channel info\n");
		JxlDecoderDestroy(dec);
		free(encoded);
		return 1;
	}
