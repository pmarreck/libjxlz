...
int main(void) {
	...
	if (!encoded || encoded_size < 2 || encoded[0] != 0xFF || encoded[1] != 0x0A) {
		fprintf(stderr, "unexpected encoded prefix\n");
		free(encoded);
		return 1;
	}
	...
	if (decoded_info.xsize != 2 || decoded_info.ysize != 2) {
		fprintf(stderr, "unexpected decoded size %u x %u\n", decoded_info.xsize, decoded_info.ysize);
		free(decoded_pixels);
		JxlDecoderDestroy(dec);
		free(encoded);
		return 1;
