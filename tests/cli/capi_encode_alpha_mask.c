...
int main(void) {
	...
	JxlPixelFormat rgba_format = {4, JXL_TYPE_UINT8, JXL_NATIVE_ENDIAN, 0};
	...
	if (JxlEncoderSetExtraChannelInfo(enc, 1, &selection_mask) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set sidecar extra info failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	if (JxlEncoderSetExtraChannelName(enc, 1, "mask", 4) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set sidecar extra name failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	if (JxlEncoderAddImageFrame(settings, &rgba_format, rgba_pixels, sizeof(rgba_pixels)) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "add image frame failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	if (JxlEncoderSetExtraChannelBuffer(settings, &extra_format, mask_pixels, sizeof(mask_pixels), 1) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set sidecar extra buffer failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	...
	if (
		decoded_info.xsize != 2 ||
		decoded_info.ysize != 2 ||
		decoded_info.alpha_bits != 8 ||
		decoded_info.num_extra_channels != 2
	) {
