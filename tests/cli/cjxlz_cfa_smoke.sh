...
BUILD_LOG="${TMPDIR}/cjxlz_cfa_build.log"
RUN_STDERR="${TMPDIR}/cjxlz_cfa_run_stderr.log"
CHECK_STDOUT="${TMPDIR}/cjxlz_cfa_check_stdout.txt"
CHECK_STDERR="${TMPDIR}/cjxlz_cfa_check_stderr.txt"
CHECK_BIN="${TMPDIR}/cjxlz_cfa_info"
CHECK_BUILD_LOG="${TMPDIR}/cjxlz_cfa_check_build.log"
INPUT_PPM="${TMPDIR}/cjxlz_cfa_input.ppm"
CFA_PGM="${TMPDIR}/cjxlz_cfa_plane.pgm"
ENCODED_JXL="${TMPDIR}/cjxlz_cfa_output.jxl"
ROUNDTRIP_PPM="${TMPDIR}/cjxlz_cfa_roundtrip.ppm"
...
printf 'P5\n2 2\n255\n' >"${CFA_PGM}"
printf '\x01\x02\x03\x04' >>"${CFA_PGM}"

if ! "${PACKAGE_OUT}/bin/cjxlz" --extra cfa:2 "${CFA_PGM}" "${INPUT_PPM}" "${ENCODED_JXL}" >"${CHECK_STDOUT}" 2>"${RUN_STDERR}"; then
	cat "${RUN_STDERR}"
	exit 1
fi
