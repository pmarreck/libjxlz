...
int main(void) {
	...
	JxlPixelFormat mask_format = {1, JXL_TYPE_UINT8, JXL_NATIVE_ENDIAN, 0};
	...
	if (JxlEncoderSetExtraChannelInfo(enc, 0, &extra) != JXL_ENC_SUCCESS) {
	...
	if (JxlEncoderSetExtraChannelBuffer(settings, &mask_format, mask_pixels, sizeof(mask_pixels), 0) != JXL_ENC_SUCCESS) {
	...
	if (encoded_size < 2 || encoded[0] != 0xFF || encoded[1] != 0x0A) {
		fprintf(stderr, "invalid codestream signature\n");
		free(encoded);
		return 1;
	}
	...
	if (decoded_info.xsize != 2 || decoded_info.ysize != 2 || decoded_info.num_extra_channels != 1 || decoded_info.alpha_bits != 0) {
