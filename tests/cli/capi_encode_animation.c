...
int main(void) {
	...
	frame_header.timecode = 0x01020304u;
	if (JxlEncoderSetFrameHeader(settings, &frame_header) != JXL_ENC_SUCCESS) {
		fprintf(stderr, "set frame header failed\n");
		JxlEncoderDestroy(enc);
		return 1;
	}
	...
	if (
		decoded_info.have_animation != 1 ||
		decoded_info.animation.tps_numerator != 24 ||
		decoded_info.animation.tps_denominator != 1001 ||
		decoded_info.animation.num_loops != 3 ||
		decoded_info.animation.have_timecodes != 1
	) {
		fprintf(stderr, "animation metadata mismatch\n");
		free(decoded_pixels);
		JxlDecoderDestroy(dec);
		free(encoded);
		return 1;
