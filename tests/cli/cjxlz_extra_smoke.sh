...
BUILD_LOG="${TMPDIR}/cjxlz_extra_build.log"
RUN_STDERR="${TMPDIR}/cjxlz_extra_run_stderr.log"
CHECK_STDOUT="${TMPDIR}/cjxlz_extra_check_stdout.txt"
CHECK_STDERR="${TMPDIR}/cjxlz_extra_check_stderr.txt"
CHECK_BIN="${TMPDIR}/cjxlz_extra_info"
CHECK_BUILD_LOG="${TMPDIR}/cjxlz_extra_check_build.log"
INPUT_PPM="${TMPDIR}/cjxlz_extra_input.ppm"
MASK_PGM="${TMPDIR}/cjxlz_extra_mask.pgm"
ENCODED_JXL="${TMPDIR}/cjxlz_extra_output.jxl"
ROUNDTRIP_PPM="${TMPDIR}/cjxlz_extra_roundtrip.ppm"
...
if ! "${PACKAGE_OUT}/bin/cjxlz" --extra selection_mask "${MASK_PGM}" "${INPUT_PPM}" "${ENCODED_JXL}" >"${CHECK_STDOUT}" 2>"${RUN_STDERR}"; then
	cat "${RUN_STDERR}"
	exit 1
fi
