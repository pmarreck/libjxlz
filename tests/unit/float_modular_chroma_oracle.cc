...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
 ...
 float value=(int((x*13+y*7+c*29)%180)-64)/255.f;unsigned bits;memcpy(&bits,&value,4);
 if(x==0&&y==0)bits=0x7f800000;
 if(x==width/3&&y==height/3)bits=0xff800000;
 if(x==width/2&&y==height/2)bits=0x7fc12345;
 if(x==width/4&&y==height/4)bits=0x80000000;
 ...
 JxlDecoderDestroy(decoder);if(frames!=1)return false;printf("pub const bytes_%u=[_]u8{",id);for(auto byte:bytes)printf("%u,",byte);printf("};\npub const pixels_%u=[_]u32{",id);for(size_t i=0;i<pixels.size();i+=4){unsigned bits;memcpy(&bits,pixels.data()+i,4);printf("0x%08x,",bits);}printf("};\n");return true;
}
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/float_modular_chroma_oracle.cc with upstream libjxl.\n");for(unsigned id=0;id<12;++id)if(!Generate(&memory,id))return 2;}
