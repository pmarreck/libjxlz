...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
 const unsigned widths[]={1,17,257,273,519,2056,65,33},heights[]={1,3,5,9,17,9,33,17};
 ...
 for(unsigned frame=0;frame<(id>=28?4:2);++frame){
  ...
  if(!(actual.flags&jxl::FrameHeader::kNoise)||actual.animation_frame.duration!=bundle.duration||actual.blending_info.mode!=(frame?info.blendmode:jxl::BlendMode::kReplace)||actual.frame_origin.x0!=info.origin.x0||actual.frame_origin.y0!=info.origin.y0){fprintf(stderr,"noise fixture settings were not emitted\n");return false;}
 }
...
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/noise_frame_oracle.cc.\n");for(unsigned id=0;id<32;++id)if(!Generate(&memory,id))return 2;}
