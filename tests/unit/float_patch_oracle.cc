...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
 ...
 for(unsigned frame=0;frame<2;++frame){
  ...
  for(unsigned c=0;c<3;++c)for(unsigned y=0;y<h;++y)for(unsigned x=0;x<w;++x){const auto bits=words[(x+y*3+c*2+frame*3)%10];memcpy(&image.PlaneRow(c,y)[x],&bits,4);}
...
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/float_patch_oracle.cc.\n");for(unsigned id=0;id<16;++id)if(!Generate(&memory,id))return 2;}
