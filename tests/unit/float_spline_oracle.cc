...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
 const bool floating=id<10||id>=20; const unsigned width=id>=20?31:13,height=id>=20?19:9,extras=(id%10)/5,channels=3+extras;
 jxl::CodecMetadata metadata;JXL_RETURN_IF_ERROR(metadata.size.Set(width,height));if(floating)metadata.m.SetFloat32Samples();else metadata.m.SetUintSamples(8);metadata.m.xyb_encoded=false;metadata.m.color_encoding=jxl::ColorEncoding::SRGB();
 ...
 jxl::CompressParams params;params.SetLossless();params.gaborish=id>=20?jxl::Override::kOn:jxl::Override::kOff;params.epf=id>=20?id%4:0;params.resampling=id>=20?(id%2?2:4):1;params.ec_resampling=params.resampling;params.patches=jxl::Override::kOff;params.speed_tier=jxl::SpeedTier::kThunder;JXL_RETURN_IF_ERROR(jxl::ParamsPostInit(&params));
 for(unsigned frame=0;frame<2;++frame){
  ...
  for(unsigned c=0;c<3;++c)for(unsigned y=0;y<h;++y)for(unsigned x=0;x<w;++x){const unsigned raw=words[(x+y*3+c*2+frame*3)%10];if(id<10)memcpy(&image.PlaneRow(c,y)[x],&raw,4);else{image.PlaneRow(c,y)[x]=((x*17+y*11+c*29+frame*31)%256)/255.f;if(id>=20&&((c==0&&x==0&&y==0)||(c==1&&x==w/2&&y==h/2)||(c==2&&x==w-1&&y==h-1))){const unsigned special=c==0?0x7f800000:c==1?0x7fc00001:0x80000000;memcpy(&image.PlaneRow(c,y)[x],&special,4);}}}
  ...
  if(!(actual.flags&jxl::FrameHeader::kSplines)||bool(actual.flags&jxl::FrameHeader::kNoise)!=bool(id%2)||actual.blending_info.mode!=(frame?info.blendmode:jxl::BlendMode::kReplace)||actual.frame_origin.x0!=info.origin.x0||actual.frame_origin.y0!=info.origin.y0||actual.encoding!=jxl::FrameEncoding::kModular||metadata.m.xyb_encoded||metadata.m.bit_depth.floating_point_sample!=floating||metadata.m.bit_depth.exponent_bits_per_sample!=(floating?8:0)||actual.loop_filter.gab!=(id>=20)||actual.loop_filter.epf_iters!=unsigned(params.epf)||actual.upsampling!=unsigned(params.resampling)){fprintf(stderr,"float reference fixture settings were not emitted\n");return false;}
 }
...
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/float_spline_oracle.cc.\n");for(unsigned id=0;id<30;++id)if(!Generate(&memory,id))return 2;}
