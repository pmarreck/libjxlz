...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
	const unsigned width=id>=20?(id%2?32:31):13,height=id>=20?(id%2?24:19):9,extras=1,channels=3+extras;
	...
	metadata.m.extra_channel_info.resize(extras);for(auto& extra:metadata.m.extra_channel_info){extra.type=jxl::ExtraChannel::kAlpha;extra.bit_depth.bits_per_sample=id>=20&&id%2?16:32;extra.bit_depth.floating_point_sample=true;extra.bit_depth.exponent_bits_per_sample=id>=20&&id%2?5:8;extra.alpha_associated=id%2;}
	jxl::BitWriter writer(memory);JXL_RETURN_IF_ERROR(jxl::WriteCodestreamHeaders(&metadata,&writer,nullptr));writer.ZeroPadToByte();
	jxl::CompressParams params;params.modular_mode=id%20>=10;params.color_transform=jxl::ColorTransform::kXYB;params.butteraugli_distance=1.0f;params.ec_distance={0.0f};params.gaborish=id>=20&&id%2?jxl::Override::kOn:jxl::Override::kOff;params.epf=id>=20?id%4:0;params.resampling=id>=20?1u<<(1+id%2):1;params.ec_resampling=id>=20&&id%3==0?8:params.resampling;params.patches=jxl::Override::kOff;params.speed_tier=jxl::SpeedTier::kThunder;JXL_RETURN_IF_ERROR(jxl::ParamsPostInit(&params));
	for(unsigned frame=0;frame<2;++frame){
		// Aligned half-float crops keep downsampled finite samples representable.
		const unsigned w=frame?(id>=20&&id%2?8:7):width,h=frame?(id>=20&&id%2?8:5):height;
		...
		if(actual.blending_info.mode!=(frame?info.blendmode:jxl::BlendMode::kReplace)||actual.frame_origin.x0!=info.origin.x0||actual.frame_origin.y0!=info.origin.y0||actual.encoding!=(id%20>=10?jxl::FrameEncoding::kModular:jxl::FrameEncoding::kVarDCT)||!metadata.m.xyb_encoded||metadata.m.bit_depth.floating_point_sample||!metadata.m.extra_channel_info[0].bit_depth.floating_point_sample||metadata.m.extra_channel_info[0].bit_depth.exponent_bits_per_sample!=(id>=20&&id%2?5:8)||actual.loop_filter.gab!=(id>=20&&id%2)||actual.loop_filter.epf_iters!=(id>=20?id%4:0)||actual.upsampling!=params.resampling||actual.extra_channel_upsampling[0]!=params.ec_resampling){fprintf(stderr,"xyb fixture id=%u frame=%u encoding=%u gab=%u epf=%u sampling=%u extra=%u exponent=%u origin=%d,%d wanted=%d,%d\n",id,frame,unsigned(actual.encoding),unsigned(actual.loop_filter.gab),unsigned(actual.loop_filter.epf_iters),unsigned(actual.upsampling),unsigned(actual.extra_channel_upsampling[0]),unsigned(metadata.m.extra_channel_info[0].bit_depth.exponent_bits_per_sample),int(actual.frame_origin.x0),int(actual.frame_origin.y0),int(info.origin.x0),int(info.origin.y0));return false;}
	}
...
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/float_xyb_extra_oracle.cc.\n");for(unsigned id=0;id<40;++id)if(!Generate(&memory,id))return 2;}
