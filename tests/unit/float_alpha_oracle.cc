...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
 const unsigned width=13,height=9,extras=1,channels=3+extras;
 ...
 for(unsigned frame=0;frame<2;++frame){
  ...
  if(actual.blending_info.mode!=(frame?info.blendmode:jxl::BlendMode::kReplace)||actual.frame_origin.x0!=info.origin.x0||actual.frame_origin.y0!=info.origin.y0||actual.encoding!=jxl::FrameEncoding::kModular||metadata.m.xyb_encoded||metadata.m.bit_depth.floating_point_sample||!metadata.m.extra_channel_info[0].bit_depth.floating_point_sample||metadata.m.extra_channel_info[0].bit_depth.exponent_bits_per_sample!=8||actual.loop_filter.gab||actual.loop_filter.epf_iters){fprintf(stderr,"float reference fixture settings were not emitted\n");return false;}
 }
...
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/float_alpha_oracle.cc.\n");for(unsigned id=0;id<10;++id)if(!Generate(&memory,id))return 2;}
