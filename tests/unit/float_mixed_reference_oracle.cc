...
jxl::Status Generate(JxlMemoryManager* memory,unsigned id){
 ...
 for(unsigned frame=0;frame<2;++frame){
  ...
  params.color_transform=frame==id/10?jxl::ColorTransform::kYCbCr:jxl::ColorTransform::kNone;
...
int main(){JxlMemoryManager memory;if(!jxl::MemoryManagerInit(&memory,nullptr))return 1;printf("// Generated by tests/unit/float_mixed_reference_oracle.cc.\n");for(unsigned id=0;id<20;++id)if(!Generate(&memory,id))return 2;}
