...
const fixture = @import("../lib/codec/float_reference_fixture.zig");
...
fn check(comptime id: usize, comptime coalescing: bool) !void {
	...
	const format = api.JxlPixelFormat{ .num_channels = 3 + id / 5, .data_type = .JXL_TYPE_FLOAT, .endianness = .JXL_LITTLE_ENDIAN, .@"align" = 0 };
	for (0..2) |_| {
		...
		inline for (0..@field(fixture, (if (coalescing) "frames_" else "layers_") ++ key)) |frame| {
			...
			for (expected, 0..) |bits, i| {
				...
				if (!coalescing or id == 0 or id == 2 or id == 5 or bits & 0x7fffffff == 0 or bits & 0x7fffffff == 0x7f800000) try std.testing.expectEqual(bits, actual) else if (bits & 0x7fffffff > 0x7f800000) try std.testing.expect(actual & 0x7fffffff > 0x7f800000) else {
...
test "floating reference layers match upstream without coalescing and after rewind" {
	inline for (0..10) |id| {
		try check(id, false);
		try check(id, true);
	}
