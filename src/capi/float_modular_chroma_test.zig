...
const fixture = @import("../lib/codec/float_modular_chroma_fixture.zig");
...
fn check(comptime id: usize) !void {
	...
	for (0..2) |_| {
		...
		if (status != api.JxlDecoderStatus.JXL_DEC_FULL_IMAGE) std.debug.print("chroma id={d} status={any}\n", .{ id, status });
		try std.testing.expectEqual(api.JxlDecoderStatus.JXL_DEC_FULL_IMAGE, status);
...
test "floating subsampled YCbCr matches upstream complete frames" {
	inline for (0..12) |id| try check(id);
}
