...
const fixture = @import("../lib/codec/float_noise_fixture.zig");
	...
	for (0..2) |_| {
		...
		inline for (0..@field(fixture, (if (coalescing) "frames_" else "layers_") ++ key)) |frame| {
			...
			for (expected, 0..) |bits, i| {
				...
				errdefer std.debug.print("noise id={d} coalescing={} frame={d} component={d} expected={x} actual={x}\n", .{ id, coalescing, frame, i, bits, actual });
...
test "floating noise matches upstream coalesced and separate layers" {
	inline for (0..10) |id| {
		try check(id, false);
		try check(id, true);
	}
}
test "integer non-XYB noise matches upstream coalesced and separate layers" {
	inline for (10..20) |id| {
		try check(id, false);
		try check(id, true);
	}
