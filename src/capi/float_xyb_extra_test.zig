...
const fixture = @import("../lib/codec/float_xyb_extra_fixture.zig");
...
fn check(comptime id: usize, comptime coalescing: bool) !void {
	...
	const format = api.JxlPixelFormat{ .num_channels = 4, .data_type = .JXL_TYPE_FLOAT, .endianness = .JXL_LITTLE_ENDIAN, .@"align" = 0 };
	for (0..2) |_| {
		...
		inline for (0..@field(fixture, (if (coalescing) "frames_" else "layers_") ++ key)) |frame| {
			...
			for (expected, 0..) |bits, i| {
				...
				errdefer std.debug.print("xyb extra id={d} coalescing={} frame={d} component={d} expected={x} actual={x}\n", .{ id, coalescing, frame, i, bits, actual });
				if ((id < 20 and !coalescing and i % 4 == 3) or bits & 0x7fffffff == 0 or bits & 0x7fffffff == 0x7f800000) try std.testing.expectEqual(bits, actual) else if (bits & 0x7fffffff > 0x7f800000) try std.testing.expect(actual & 0x7fffffff > 0x7f800000) else {
					const value: f32 = @bitCast(actual);
					try std.testing.expect(std.math.isFinite(value));
					// Existing XYB output controls allow 1e-4 for color conversion.
					// Extra-channel arithmetic keeps the tighter binary32 bound.
					try std.testing.expectApproxEqAbs(@as(f32, @bitCast(bits)), value, if (i % 4 < 3) @as(f32, 0.0001) else @as(f32, 0.000001));
				}
			}
		}
...
test "XYB color with nonfinite floating alpha matches upstream and rewind" {
	inline for (0..20) |id| {
		try check(id, false);
		try check(id, true);
	}
...
test "XYB floating extra filters and resampling match upstream" {
	inline for (20..40) |id| {
		try check(id, false);
		try check(id, true);
	}
