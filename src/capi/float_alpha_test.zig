...
const fixture = @import("../lib/codec/float_alpha_fixture.zig");
	...
	for (0..2) |_| {
		...
		inline for (0..@field(fixture, (if (coalescing) "frames_" else "layers_") ++ key)) |frame| {
			...
			for (expected, 0..) |bits, i| {
				...
				if ((!coalescing and i % 4 == 3) or bits & 0x7fffffff == 0 or bits & 0x7fffffff == 0x7f800000) try std.testing.expectEqual(bits, actual) else if (bits & 0x7fffffff > 0x7f800000) try std.testing.expect(actual & 0x7fffffff > 0x7f800000) else {
...
test "integer color with nonfinite floating alpha matches upstream and rewind" {
	inline for (0..10) |id| {
		try check(id, false);
		try check(id, true);
	}
