...
const fixture = @import("float_spline_fixture.zig");
...
test "float spline frames preserve reference storage" {
	try replay(std.testing.allocator);
}
test "float spline reference allocation failures release partial state" {
