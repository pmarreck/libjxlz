...
const fixture = @import("reference_fixture.zig");
fn checkFixture(allocator: std.mem.Allocator, data: []const u8, expected: []const u8, id: usize) !void {
	...
	while (offset < data.len) {
		...
		if (dec.frame_header.is_last) {
			...
			try std.testing.expectEqual(13, image.xsize);
			try std.testing.expectEqual(9, image.ysize);
			try std.testing.expectEqual(3 + id / 5, image.channels);
			for (0..9) |y| for (0..13) |x| for (0..image.channels) |c| {
				const pixel: i32 = @intFromFloat(@round(std.math.clamp(image.rowConst(y, c)[x], 0, 1) * 255));
				try std.testing.expect(@abs(pixel - @as(i32, expected[(y * 13 + x) * image.channels + c])) <= 1);
			};
		}
	}
...
test "reference frame session matches upstream cropped blend pixels" {
	try check(std.testing.allocator, 10);
}
fn one(allocator: std.mem.Allocator) !void {
	try check(allocator, 1);
}
test "reference frame session allocation failures release state" {
