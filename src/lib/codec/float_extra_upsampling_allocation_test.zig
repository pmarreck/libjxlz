...
const fixture = @import("float_extra_upsampling_fixture.zig");
fn decode(allocator: std.mem.Allocator) !void {
	const data = &fixture.bytes_2;
	...
	try std.testing.expectEqual(@as(usize, 12), image.xsize);
	try std.testing.expectEqual(@as(usize, 8), image.ysize);
	for (0..image.ysize) |y| for (0..image.xsize) |x| for (0..4) |c| {
		const expected = fixture.float_2[(y * image.xsize + x) * 4 + c];
...
test "nonfinite extra upsampling allocation failures release partial rendered images" {
