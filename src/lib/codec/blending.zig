...
const Finite = struct {
	pub const Fixed = jxl.base.soft_float.Fixed;
	pub const zero = Fixed.zero;
	pub const add = jxl.base.soft_float.add;
	pub const sub = jxl.base.soft_float.sub;
	pub const mul = jxl.base.soft_float.mul;
	pub const div = jxl.base.soft_float.div;
	pub const fromInt = jxl.base.soft_float.fromInt;
	pub const cmp = jxl.base.soft_float.cmp;
	pub fn negative(value: Fixed) bool {
		return value.m < 0;
	}
	pub fn divideAlpha(value: Fixed, alpha: Fixed) Fixed {
		return if (alpha.m <= 0) zero else div(value, alpha);
	}
};
pub const pixel = Implementation(Finite).pixel;
pub const pixelBinary32 = Implementation(@import("../base/binary32.zig")).pixel;
fn Implementation(comptime sf: type) type {
	return struct {
		...
		fn clamp(value: sf.Fixed) sf.Fixed {
			return if (sf.negative(value)) sf.zero else if (sf.cmp(value, sf.fromInt(1)) > 0) sf.fromInt(1) else value;
		}
		...
		fn compute(bottom: sf.Fixed, top: sf.Fixed, bga: sf.Fixed, fga: sf.Fixed, info: Info, associated: bool, is_alpha: bool) sf.Fixed {
			...
			return sf.divideAlpha(sf.add(sf.mul(top, fa), sf.mul(sf.mul(bottom, bga), remainder)), alpha);
		}
		...
		pub fn pixel(bg: []const sf.Fixed, fg: []const sf.Fixed, output: []sf.Fixed, color: Info, extras: []const Extra) jxl.base.status.JxlError!void {
			...
			const top = if (below(info.mode)) bg else fg;
			const alpha = if (usesAlpha(info.mode)) info.alpha_channel else 0;
			const bga = if (usesAlpha(info.mode)) bottom[3 + alpha] else sf.zero;
			const fga = if (usesAlpha(info.mode)) top[3 + alpha] else sf.zero;
