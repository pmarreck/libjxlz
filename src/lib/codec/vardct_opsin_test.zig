...
const fixture = @import("vardct_opsin_fixture.zig");
...
test "custom opsin frames match upstream RGB output" {
	try check(std.testing.allocator, 0, 3);
}

fn checkOne(allocator: std.mem.Allocator) !void {
	try check(allocator, 0, 1);
}
test "custom opsin frame allocation failures release partial state" {
	try std.testing.checkAllAllocationFailures(std.testing.allocator, checkOne, .{});
}

test "custom opsin frame rejects every truncated section prefix" {
...
test "custom opsin headers preserve explicit all-zero values" {
