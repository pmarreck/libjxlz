...
const fixture = @import("float_upsampling_frame_fixture.zig");
fn decode(allocator: std.mem.Allocator) !void {
	const data = &fixture.bytes_8;
	...
	try std.testing.expectEqual(@as(usize, 103), image.xsize);
	try std.testing.expectEqual(@as(usize, 71), image.ysize);
	for (0..image.ysize) |y| for (0..image.xsize) |x| for (0..3) |c| {
		const expected = fixture.float_8[(y * image.xsize + x) * 3 + c];
...
test "nonfinite upsampling allocation failures release partial rendered images" {
