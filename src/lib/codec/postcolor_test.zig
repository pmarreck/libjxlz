...
const fixture = @import("postcolor_fixture.zig");
...
test "post-color session matches upstream cropped blend pixels" {
	try check(std.testing.allocator, 20);
}
fn one(allocator: std.mem.Allocator) !void {
	try check(allocator, 1);
}
test "post-color session allocation failures release state" {
