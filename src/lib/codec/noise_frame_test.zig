...
const fixture = @import("noise_frame_fixture.zig");
		...
		if (dec.frame_header.is_last) {
			...
			for (0..image.ysize) |y| for (0..image.xsize) |x| for (0..image.channels) |c| {
				...
				if (@abs(pixel - @as(i32, wanted)) > 1) {
					std.debug.print("noise id={d} x={d} y={d} c={d} actual={d} expected={d}\n", .{ id, x, y, c, pixel, wanted });
...
test "noise frame session matches upstream cropped blend pixels" {
...
test "noise frame session allocation failures release state" {
