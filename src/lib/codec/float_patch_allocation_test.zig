...
const fixture = @import("float_patch_fixture.zig");
	...
	for (0..2) |_| {
		...
		while (offset < data.len) {
			...
			if (dec.frame_header.is_last) {
				...
				try std.testing.expectEqual(@as(usize, 16), image.xsize);
				try std.testing.expectEqual(@as(usize, 12), image.ysize);
				for (0..image.ysize) |y| for (0..image.xsize) |x| for (0..3) |c| {
					try std.testing.expectEqual(fixture.pixels_0[(y * image.xsize + x) * 3 + c], @as(u32, @bitCast(image.rowConst(y, c)[x])));
				};
			}
		}
...
test "float patch replay preserves storage and replaces owned slots" {
	try replay(std.testing.allocator);
}
test "float patch allocation failures release partial state" {
