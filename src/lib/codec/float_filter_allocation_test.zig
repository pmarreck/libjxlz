...
fn decode(allocator: std.mem.Allocator) !void {
	...
	try std.testing.expectEqual(@as(usize, 31), image.xsize);
	try std.testing.expectEqual(@as(usize, 19), image.ysize);
	for (0..image.ysize) |y| for (0..image.xsize) |x| for (0..3) |c| {
		const expected = fixture.float_7[(y * image.xsize + x) * 3 + c];
...
test "nonfinite filter allocation failures release partial rendered images" {
