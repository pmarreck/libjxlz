//! Gaborish and EPF over Fixed/binary32 planes with mirrored image borders.
const std = @import("std");
const original = @import("../root.zig").base.soft_float;
...
const Finite = struct {
	...
	pub const sub = original.sub;
	pub const mul = original.mul;
	pub const div = original.div;
	pub const fromInt = original.fromInt;
	pub const parse = original.parse;
	pub const cmp = original.cmp;
	pub const neg = original.neg;
	pub const fromF32 = @import("../base/float.zig").loadFloat32Fixed;
	pub fn negative(v: Fixed) bool {
		return v.m < 0;
	}
	pub fn abs(v: Fixed) Fixed {
		return if (v.m < 0) original.neg(v) else v;
	}
	pub fn nonnegativeWeight(v: Fixed) Fixed {
		return v;
	}
	pub fn skipWeight(v: Fixed) bool {
		return v.m <= 0;
	}
};
pub const Binary32 = Implementation(@import("../base/binary32.zig"));
pub const Params = Implementation(Finite).FilterParams;
pub const Image = Implementation(Finite).PixelImage;
pub const inverseSigma = Implementation(Finite).inverseSigma;
pub const gaborish = Implementation(Finite).gaborish;
pub const epf = Implementation(Finite).epf;
fn Implementation(comptime sf: type) type {
	return struct {
		...
		pub const FilterParams = struct {
			...
			pub fn fromHeader(header: @import("../root.zig").codec.loop_filter.LoopFilter) JxlError!FilterParams {
		...
		pub const PixelImage = struct {
			...
			height: usize,
			data: []sf.Fixed,
			pub fn validate(self: PixelImage) JxlError!void {
			...
			fn at(self: PixelImage, c: usize, x: isize, y: isize) sf.Fixed {
		...
		pub fn gaborish(allocator: std.mem.Allocator, image: PixelImage, params: FilterParams) JxlError!void {
			try image.validate();
			...
			for (params.gaborish, 0..) |w, c| {
				...
				const magnitude = if (sf.negative(denominator)) sf.neg(denominator) else denominator;
		...
		pub fn epf(allocator: std.mem.Allocator, image: PixelImage, params: FilterParams, sigma: []const sf.Fixed, stage: u2) JxlError!void {
			try image.validate();
			...
			for (0..image.height) |uy| for (0..image.width) |ux| {
				...
				for (offsets) |offset| {
					var sad = sf.zero;
					for (0..3) |c| {
						var channel_sad = sf.zero;
						for (patch) |p| {
							const delta = sf.sub(image.at(c, x + p.x, y + p.y), image.at(c, x + p.x + offset.x, y + p.y + offset.y));
							channel_sad = sf.add(channel_sad, sf.abs(delta));
						}
						sad = sf.add(sad, sf.mul(channel_sad, params.channel_scale[c]));
					}
					const weight = sf.nonnegativeWeight(sf.add(one, sf.mul(sad, multiplier)));
					if (sf.skipWeight(weight)) continue;
