...
const sampling = @import("upsampling.zig");
const Finite = struct {
	pub const Fixed = original.Fixed;
	pub const fromInt = original.fromInt;
	pub const bits = @import("../base/fixed_display.zig").bits;
	pub const fromSample = @import("float_samples.zig").toFixed;
};
const Float = struct {
	pub const Fixed = u32;
	pub fn bits(value: u32) u32 {
		return value;
	}
	pub fn fromSample(raw: i32, depth: jxl.codec.image_metadata.BitDepth) jxl.base.status.JxlError!u32 {
...
fn Implementation(comptime sf: type) type {
	return struct {
		const up = if (sf == Float) sampling.Binary32 else sampling;
		const Plane = struct { width: usize = 0, height: usize = 0, data: []sf.Fixed = &.{} };
		pub fn finish(dec: *jxl.codec.dec_frame.FrameDecoder, output: if (sf == Float) jxl.codec.vardct_filters.Binary32.PixelImage else jxl.codec.vardct_filters.Image) !void {
			...
			for (planes[3..], 0..) |*plane, e| {
				...
				for (0..channel.h) |y| for (channel.rowConst(y)[0..channel.w], plane.data[y * channel.w ..][0..channel.w]) |raw, *value| {
					value.* = try sf.fromSample(raw, metadata.extra_channel_info[e].bit_depth);
				};
			...
			if (dec.patches) |*dictionary| {
				...
				const apply = if (sf == Float) @TypeOf(dictionary.*).applyBinary32 else @TypeOf(dictionary.*).apply;
				try apply(dictionary, .{ .width = output.width, .height = output.height, .channels = channels, .data = values }, dec.references orelse return error.GenericError, extras);
