...
pub fn render(dec: *jxl.codec.dec_frame.FrameDecoder) !void {
	...
	var effects = fh.upsampling != 1 or fh.loop_filter.gab or fh.loop_filter.epf_iters != 0 or dec.patches != null or dec.noise.hasAny() or fh.color_transform == .ycbcr;
	...
	if (!fh.chroma_subsampling.is444()) {
		...
		for (image.channels.items[0..3], 0..) |channel, c| {
			...
			@memcpy(output.data[c * width * height ..][0 .. width * height], sampled);
		}
	} else {
