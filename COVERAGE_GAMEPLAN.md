...
Remaining decode coverage (reconciled against indexed Zig source and call sites
on 2026-09-04; component presence does not establish full-format coverage):

- **VarDCT frame integration and remaining coefficient processing.**
  `decodeFrame` still returns `error.Unsupported`
  ...
  DCT2, DCT4, DCT4x8, AFV, DCT 8–64 square, and rectangular
  8×16/8×32/16×32/32×64 library tables (distance-band `GetQuantWeights` in
  randomz Fixed). As of 2026-09-04, the quantizer parses all U32 forms and
  derives Fixed coefficient scales. Modular AC metadata now decodes adaptive
  quantization, all 27 strategy geometries, EPF sharpness and CfL maps; natural
  coefficient orders through DCT256 match digests generated by compiled C++.
  DC quantization stays in Fixed until XYB display. These components are not
  yet dispatched by `decodeFrame`. On 2026-09-05, custom coefficient orders
  gained a shared-reader adaptation of the existing TOC Lehmer decoder;
...
- **JPEG reconstruction (`jbrd`).** The C API recognizes the reserved box name;
  no JPEG reconstruction implementation was found. Recognition alone does
  not prove its payload or reconstruction semantics.
