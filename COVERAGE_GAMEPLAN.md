...
- **VarDCT in its entirety.** `decodeFrame` still returns `error.Unsupported`
  ...
  randomz Fixed). Still missing: DCT128+, raw table compute, the quantizer
  and adaptive quant field, coefficient order tables, AC strategy (DCT128
  through DCT256), chroma-from-luma, and the inverse DCT.
