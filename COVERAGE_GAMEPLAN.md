...
- **VarDCT in its entirety.** `decodeFrame` still returns `error.Unsupported`
  ...
  yet dispatched by `decodeFrame`. Still missing: DCT128+ dequant tables,
  raw table compute, custom coefficient permutations, DC-global contexts and
  CfL parameters, DC/AC coefficient groups, and the inverse transforms.
