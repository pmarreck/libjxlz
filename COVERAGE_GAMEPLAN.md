...
- **VarDCT in its entirety.** `decodeFrame` still returns `error.Unsupported`
  ...
  DCT2, DCT8, DCT16, DCT32, and rectangular 8×16/8×32/16×32 library tables
  (distance-band `GetQuantWeights` in randomz Fixed). Still missing: DCT64+,
  DCT4/DCT4x8/AFV/raw table compute, the quantizer and adaptive quant field,
  coefficient order tables, AC strategy (DCT64 through DCT256 plus Hornuss
...
1. [ ] `DequantMatrices` in full (`decode` all modes except raw; `EnsureComputed` identity/DCT2/DCT 8/16/32 square+rectangular; DCT64+ / DCT4/AFV/raw compute still open).
