...
- [ ] Port the next upstream entropy-planning slice: build/cluster raw-value histograms before uint-config choice so histogram clustering is no longer constrained by pre-encoded token alphabets or same-config families
